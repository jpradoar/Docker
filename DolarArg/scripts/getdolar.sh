...
#	echo "dolarhoy"
	sleep 10
        bancogalicia=$(curl -s 'https://dolaraldia.com/bancosargentina/banco-galicia/' -H 'authority: www.preciodolar.com.ar' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/68.0.3440.106 Safari/537.36' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' |grep CotizacionBanco |grep "Banco Galicia"|cut -d"/" -f4- |cut -d"<" -f1)   &&  echo "'{"source": "BancoGalicia", "price" : '$bancogalicia', "@timestamp" : "'$(date --utc +%FT%T.%3NZ)'"}'"
